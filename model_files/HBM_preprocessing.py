	...
	def get_dustlaw(self):
		...
		def get_censored_data(self):
			...
			if self.choices['analysis_parameters']['CensoredData']:
				...
				DF_M[0]['BV'] = DF_M[0]['B']-DF_M[tref]['V']
				RetainedSNe = list(DF_M[0][DF_M[tref]['BV'].abs()<0.3].index)
				CensoredSNe = list(DF_M[0][(DF_M[tref]['BV'].abs()>=0.3) & (DF_M[0]['BV'].abs()<CensoredCut)].index)
				ExcludedSNe = list(DF_M[0][DF_M[tref]['BV'].abs()>=CensoredCut].index)
