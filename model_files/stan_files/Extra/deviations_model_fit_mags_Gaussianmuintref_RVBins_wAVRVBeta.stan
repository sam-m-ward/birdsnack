...
parameters {
	...
	vector<lower=0>[N_AVRVBeta_dists] eta_sig_RV_beta;          //Transform of RV population dispersion hyperparameter
	vector<lower=0,upper=pi()/2>[N_AVRVBeta_dists] beta0_tform;  //beta0 for muRV = muRVmin + 1/(beta0+beta1*AVs)
	vector<lower=0,upper=pi()/2>[N_AVRVBeta_dists] beta1_tform;  //beta1
...
transformed parameters {
	...
	//AVRVBeta
	vector<lower=0>[N_AVRVBeta_dists] sig_RV_beta;              //RV population dispersion Hyperparameter
	vector<lower=1/(muRVmax-muRVmin)>[N_AVRVBeta_dists] beta0;  //beta0 for muRV = muRVmin + 1/(beta0+beta1*AVs)
	vector<lower=0>[N_AVRVBeta_dists] beta1;                    //beta1
	...
	real<upper=0> alpha;
	real<lower=muRVmin,upper=muRVmax> mu_RV_beta;
	//real<lower=0> nu;
	...
	//AVRVBeta
	sig_RV_beta  = eta_sig_RV_beta*disp_sigmaRV;
	beta0        = a_sigma_beta0*tan(beta0_tform)+1/(muRVmax-muRVmin);
	beta1        = a_sigma_beta1*tan(beta1_tform);
	//nu      = tan(nu_tform);
	...
	for (s in 1:S) {
		...
		if (RVstyle==0) {
			...
			mu_RV_beta = muRVmin+pow(beta0[map_RVBin_to_RVHyp[RV_bin_vec[s]]] + beta1[map_RVBin_to_RVHyp[RV_bin_vec[s]]]*AVs[s],-1);
			alpha      = (RVsmin - mu_RV_beta)/sig_RV_beta[map_RVBin_to_RVHyp[RV_bin_vec[s]]];
			RVs[s]     = mu_RV_beta - sig_RV_beta[map_RVBin_to_RVHyp[RV_bin_vec[s]]] * ( inv_Phi ( nuRVs[s] * Phi (-alpha) ) );
		}
	}
...
model {
	...
	//AVRVBeta
	eta_sig_RV_beta~ std_normal();
	beta0_tform    ~ uniform(0,pi()/2);
	beta1_tform    ~ uniform(0,pi()/2);
