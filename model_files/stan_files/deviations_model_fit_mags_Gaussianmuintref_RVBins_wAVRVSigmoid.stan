data {
	...
	real a_sigma_beta0;
	real a_sigma_beta1;
	real a_sigma_muAVsigmoid;
	real a_sigma_sigAVsigmoid;
...
parameters {
	...
	vector<lower=0,upper=1>[N_AVRVSigmoid_dists] etaB_sigmoid;  //Bsig
...
transformed parameters {
	...
	vector<lower=0>[N_AVRVSigmoid_dists] B_sigmoid;  //Bsig
	//vector<lower=0,upper=(muRVmax-A_sigmoid)*(1+exp(-muAV_sigmoid/sigAV_sigmoid))>[N_AVRVSigmoid_dists] B_sigmoid;  //Bsig
	...
	for (n in 1:N_AVRVSigmoid_dists) {
		B_sigmoid[n] = etaB_sigmoid[n]*(muRVmax-A_sigmoid[n])*(1+exp(-muAV_sigmoid[n]/sigAV_sigmoid[n]));
	}
